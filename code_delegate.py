...
_ASK_PREAMBLE = (
    "你在某個既有專案的根目錄，**只有唯讀權限（Read／Grep／Glob）**。"
    "請**先讀這個專案的 README.md**（若有 CLAUDE.md 也一起讀）來掌握結構、版本與慣例；"
    "**優先採用文件裡已寫明的資訊**，文件沒寫的再去翻程式碼，不用整庫亂找。\n"
    "**重要：不要嘗試修改任何檔案、也不要請求寫入授權。** 就算問題提到「改／修／換」，"
    "你也只負責**確認可行性並說明會怎麼改（可附檔案行號或 diff）**；實際動手是後續另一個流程、不歸你做。"
    "請簡潔作答。\n\n以下是要回答的問題：\n")
