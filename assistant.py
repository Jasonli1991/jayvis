...
_IMG_INSTRUCTION = (
    "## 你具備生圖能力\n"
    "可在回覆中加一行 `[[圖：<英文畫面描述>]]`（梗圖用 `[[圖：<英文畫面>|<字幕>]]`）。"
    "這行使用者『看不到』；圖會以『另一則訊息』出現在你文字之後。\n"
    "- 標記放在整則回覆的『最後、自己一行』。你的文字本身要能獨立成立——"
    "**不要**寫「如下圖」「這是梗圖：」「可以玩這種反差：」這種指著圖、或以冒號結尾的句子"
    "（萬一圖沒生出來會很怪）。\n"
    "- 使用者『明確要圖』（畫／梗圖／圖／插圖／海報／logo…）時一定要生，直接給圖、別只給文字概念。\n"
    "- 一般文字問答就好好用文字回答；**除非配圖真的讓答案更清楚，否則不要硬塞圖、也不要主動提議做圖**。\n"
    "- 梗圖用 `|` 分隔：左邊只描述畫面（別把字寫進畫面），右邊放字幕（可中文），"
    "系統會把字幕用清楚字型疊上（別靠生圖模型寫字，會亂碼）。"
    "例：`[[圖：a Falcon 9 rocket landing on a drone ship, exhaust replaced by a red crashing stock chart|硬著陸]]`\n"
    "- 畫面描述用**英文**（Pollinations 對英文最準）；一則最多一張。"
)
