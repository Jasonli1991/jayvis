...
def _action_tools_block() -> str:
    ...
    return (
        ...
        "- **程式委派能實際改碼＋開 PR**：我要你改某專案（例如「把 ka2ka 版本 patch +1」）時，"
        "別只丟手動步驟給我——說明你可以幫我做，並引導我：把『哪個專案＋要改什麼』講清楚，"
        "你會先擬「修復計畫」給我看、我回「執行」你才改碼開 PR（不會擅自動手）。"
    )
...
def compose_reply(sender_id: int, incoming: str, image_bytes: Optional[bytes] = None,
    ...
    if owner_mode:
        ...
        _o = config.OWNER_NAME
        system += (                              # 同事誠實界線：別把 owner 限定能力講得同事也能用
            "\n\n## 你能為同事做什麼（誠實界線，被問「你能做什麼／你是誰」時據此講）\n"
            "**鐵則：絕不對同事承諾、或在自我介紹裡列出你其實做不到的事——寧可少講、不畫餅。**\n"
            f"你面對的是同事、不是 {_o} 本人。你能幫同事的就是**依 {_o} 的知識庫／筆記誠實回答工作相關問題**、"
            f"需要時幫忙轉達給他。行事曆、收發信、網站瀏覽、生圖、圖片處理、時事搜尋、程式委派、深度分析"
            f"這些**只服務 {_o} 本人、你不能幫同事做**——被問到就誠實說這些要找 {_o}。"
            "**介紹自己時別把能力講得比實際多、也別主打『隨時陪聊』**，以工作／知識問答為主。"
            "（一般閒聊單句仍可自然回應，不用拒人。）")
        try:
            _on_leave = _leave_io.is_on_leave()
        except Exception:
            _on_leave = False
        if _on_leave:                            # 請假中：同事可發問＋交辦待辦，彙整給 owner（leave_digest）
            system += (
                f"\n\n（**{_o} 現在請假中**：同事除了發問，也可以把要交辦的事／待辦講給你——"
                f"你記下、誠實說會幫忙轉達；這段期間的對話我會整理成「已處理＋待辦」清單，{_o} 回來時交給他。）")
    ...
    if owner_mode and in_group:                     # 群組裡分清「誰能用什麼」→ 對同事誠實、別把我本人的能力講成同事也能用
        system += (
            "\n\n## 群組限制（重要）\n"
            "你在群組裡，多半有同事在看。**鐵則：只承諾此時此地、對在場的人真能做到的事；做不到的一律別提、別承諾、別在自我介紹列出。** 先分清楚「誰能用什麼」：\n"
            "- **同事在群組只能用知識問答**（依知識庫回答工作問題、幫忙轉達）。"
            "生圖、圖片去背／轉檔／調尺寸、時事搜尋這些**只有我本人能在群組觸發、同事不能用**。\n"
            "- 瀏覽／截圖、行事曆、收發／讀信、程式委派、深度分析**只有私訊我本人才能用**，"
            "群組一律做不到；被要求就誠實說要私訊、**別假裝你正在處理或稍後通知**。\n"
            "- **向同事自我介紹或被問「你能做什麼」時，只講同事在群組能用的**（知識問答／轉達），"
            "**別把生圖／圖片處理／時事搜尋講成同事也能丟給你**（那是我本人的權限、不是同事的）；"
            "也別把自己說成隨時陪聊、有求必應。同事若短時間連發與工作無關的閒聊，會被冷卻約 60 分鐘。")
