...
**判斷要保守**：單純陳述、計畫、心情、抱怨、聊到某件事（例如「原本今天要去練背」「最近好忙」「昨天去看電影」）都**不是操作需求** → 正常用繁體中文回答、**絕不要輸出 JSON**。**程式開發相關（git／PR／commit／版本號／bump／部署／改碼／某專案要怎麼改）也不是行事曆或郵件操作**（那屬「程式委派」範疇）→ 同樣絕不輸出 JSON。拿不準就當作不是操作。'''
