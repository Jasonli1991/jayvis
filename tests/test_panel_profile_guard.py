...
def test_profile_post_rejects_all_empty(tmp_path, monkeypatch):
    ...
    # 整份空白（assistant_name 是衍生的「 的搭檔」，不該讓防呆失效）
    payload = {"owner_name": "", "title": "", "company": "", "assistant_name": " 的搭檔",
...
def test_profile_is_empty_ignores_derived_assistant_name():
    assert app_mod._profile_is_empty({"assistant_name": " 的搭檔", "projects": [], "team": [], "bosses": []}) is True
