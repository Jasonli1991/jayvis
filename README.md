...
└── tests/              # pytest 測試套件（811 passed / 5 skipped）
```
...
目前 **811 passed / 5 skipped**。5 個 skip 屬正常：需 macOS 真實 Calendar/Mail（`RUN_CALENDAR=1` / `RUN_MAIL=1`）、live LLM（`RUN_LIVE_LLM=1`）、或本機未裝 LibreOffice（文件轉檔）／onnxruntime（rembg 去背）才會跑。測試用 `pytest-randomly`（順序無關），KB 與 group_memory 在測試中皆導向暫存路徑，不污染正式資料。
