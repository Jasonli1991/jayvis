...
> **JAYVIS** 是一個自架的個人 Telegram AI 助理。當你不在線（或請假）時，同事可以**私訊它**或在工作群組**「@ 它」**，它會用**你的口吻**、根據**你自己的知識庫**（Obsidian 筆記、GitHub commits、Telegram 對話）誠實回答 —— 沒把握就老實說、不編造。附帶一個桌面**控制台**把所有設定點一點就好，**全部跑在本機**（單一檔 SQLite，不需要任何資料庫伺服器），也很容易換個人就變成「他的」助理。
...
| **本人 / 同事 / 群組三種模式** | 本人＝坦白的私人助理；同事＝誠實的「助理」人設，不冒充你、不編造私事；群組＝被 @ 才回、帶群組脈絡。 |
